...
ON d.id = pr.id_departamento
ORDER BY d.nombre ASC, pe.apellido1 ASC, pe.apellido2 ASC, pe.nombre ASC;
...
SELECT g.nombre AS grau, a.tipo AS tipus, SUM(creditos) AS total_creditos
FROM grado g
JOIN asignatura a
ON a.id_grado = g.id
...
SELECT c.anyo_inicio, COUNT(DISTINCT id_alumno) AS total
...
SELECT pe.id, pe.nombre, pe.apellido1, pe.apellido2, COUNT(a.id) AS total
FROM persona pe
JOIN profesor pr
...
LEFT JOIN asignatura a
ON a.id_profesor = pr.id_profesor
WHERE id_departamento IS NOT NULL AND a.id_profesor IS NULL;
